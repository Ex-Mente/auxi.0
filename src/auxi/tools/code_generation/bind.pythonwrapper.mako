...
def shouldWrapFunction(f):
  if f.py_combineGroup is not None:
    return True
  if f.private or f.custom_constructor == "true-no-wrap":
    return False
...
% for f in c.functions:
    % if f.private == "true" or f.name == c.name:
<% continue %>
    % endif
<%
  param_count = 0
...
##/////////////////// NAMESPACE ${c.name} ///////////////////
% for f in c.functions:
% if f.name != c.name and shouldWrapFunction(f):
    <%
...
<% continue %>
    ...
    ${func_signature} { return ${f.returnType}(this->get_override("${f.name}")(${func_call_params})); }
        % endif
    % elif shouldWrapFunction(f) and f.py_combineGroup not in py_combineGroups:
    <%
        if f.py_combineGroup is not None:
            py_combineGroups[f.py_combineGroup] = {}
        ...
        for p in f.params:
            if isTypeList(p[1]):
                ...
                if p[4] is not None:
                    combGroupTypeList = []
                    if f.py_combineGroup is not None:
                        for f_i in c.functions:
                            print(f.name, f.params[2][0], f.params[2][1], f.params[2][2], f.params[2][0], f.params[2][4])
                            if f_i.py_combineGroup == f.py_combineGroup:
                                for p_i in f.params:
                                    if p_i[4] == p[4]:
                                        print(p_i[1], p_i[0], p[0])
                                        combGroupTypeList.append((p_i[1], params_cpp + p_i[0] + "_" + p[4] + ", "))
                        py_combineGroups[f.py_combineGroup][p[0]] = combGroupTypeList
                        ptype = "PyObject"
                        for gpar, gtypes in py_combineGroups[f.py_combineGroup].items():
                            for t in gtypes:
                                param_prep += gpar + "_" + t[0] + ";\n"
                params_cpp += p[0] + ", "
                params += ptype + " " + p[0] + ", "
...
%>
    ...
    if f.forceCaps == "true":
        ...
        py_func_name = f.name[0].lower() + f.name[1:]
    %>
      % if f.name == c.name and c.abstract != "true":
        <%
            init_param_str = ""
            init_param_type_str = ""
            for p in f.params:
                if p[2] is None:
                    ...
                    init_param_type_str += p[1] + ", "
            if init_param_type_str != "":
                init_param_str = init_param_str + "optional<" + init_param_type_str[:-2] + ">"
        %>
    .def(init<${init_param_str} >())
      % elif f.private != "true" and f.name not in [c.constructor_init_function,c.destructor_clean_function, c.name]:
        % if f.returnType[-1:] == "*":
          % if f.virtual == "required":
	//.def("${py_func_name}", pure_virtual(make_function(&${func_name}, return_internal_reference<1>())), ${doc})${func_end_char}
    .def("${py_func_name}", &${func_name}, return_internal_reference<1>(), ${doc})${func_end_char}
          % elif f.virtual == "true":
    //.def("${py_func_name}", make_function(&${func_name}, return_internal_reference<1>()), make_function(&${c.name}Wrapper::default_${f.name}, return_internal_reference<1>()), ${doc})${func_end_char}
    .def("${py_func_name}", &${func_name}, return_internal_reference<1>(), ${doc})${func_end_char}
          % else:
...
<%
    ...
    # py property name
    import re
    def replaceCapsWith_s(name):
        s1 = re.sub('(.)([A-Z][a-z]+)', r'\1_\2', name)
        return re.sub('([a-z0-9])([A-Z])', r'\1_\2', s1).lower()
    py_prop_name = replaceCapsWith_s(p.name)
    if p.forceCaps == "true":
        py_prop_name = getPythonFriendlyName(py_prop_name[0].upper() + py_prop_name[1:])
    else :
        py_prop_name = getPythonFriendlyName(py_prop_name[0].lower() + py_prop_name[1:])
    if p.isList:
        if p.plural is None:
            py_prop_name = py_prop_name[:-5] + "s"
        else:
            py_prop_name = p.plural
%>
	% if p.isList:
